...
const FlightInfo = ({ flightData }) => {
  ...
  const cityName =
    flightData['airportFromID.city_en'] || flightData['airportToID.city_en'];
  const localTime = moment(new Date(timeOnShedule)).format('HH:mm');

  let status;
  switch (flightData.status) {
    case 'LN':
      status = `Landed at ${moment(new Date(flightData.actual)).format(
        'HH:mm',
      )}`;
      break;
    case 'DP':
      status = `Departed at ${moment(
        new Date(flightData.timeTakeofFact),
      ).format('HH:mm')}`;
      break;
    case 'DL':
      status = `Delayed till ${moment(new Date(flightData.actual)).format(
        'HH:mm',
      )}`;
      break;
    case 'CX':
  ...
  const dTermStyle =
    term === 'D' ? <span className="d-term">{term}</span> : <span>{term}</span>;

  return (
    <tr className="table__body-row">
      <td className="terminal">{dTermStyle}</td>
